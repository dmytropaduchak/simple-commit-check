...
import { scanTitle, type Finding } from "./rules";

const MARKER = "<!-- simple-commit-check -->";
const NAME = "Simple Commit Check";
...
async function run(): Promise<void> {
  ...
  const minLength = Number(core.getInput("min-length") || "10");
  const requireConventional = (core.getInput("require-conventional") || "false").toLowerCase() === "true";
  const findings = scanTitle(title, minLength, requireConventional);
  ...
  const shouldFail =
    failOn === "high"
      ...
      : failOn === "medium"
        ...
        : false;
  if (shouldFail) core.setFailed(`simple-commit-check: ${findings.length} finding(s) (fail-on=${failOn})`);
